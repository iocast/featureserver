<xsl:stylesheet version="1.0"
	...
	<xsl:key name="update-by-hstore" match="./*[local-name()='Property']" use="./*[local-name()='Name']"/>
			...
			<Statement>
				<xsl:choose>
					<xsl:when test="$version='1.1.0'">
						...
						<xsl:variable name="total-groups" select="count(./*[local-name()='Property'][generate-id() = generate-id(key('update-by-hstore', ./*[local-name()='Name'])[1])])" />
						
						<xsl:for-each select="./*[local-name()='Property'][count(. | key('update-by-hstore', ./*[local-name()='Name'])[1]) = 1]">
							<xsl:sort select="./*[local-name()='Name']" />
							<xsl:variable name="total-in-group" select="count(key('update-by-hstore', ./*[local-name()='Name']))" />
							...
							<xsl:choose>
								...
								<xsl:when test="fs:is_hstore($name)">
									...
									<xsl:for-each select="key('update-by-hstore', ./*[local-name()='Name'])">
